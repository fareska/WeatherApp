...
router.get('/city/:cityName', function(req, res){
    ...
    function (err,cityData){//axios.get didn't work for me ????????????????????????
...
router.get('/cities', function(req, res){
    City.find({}, (err, data)=>{res.send(data)})
})

//This route should save a new City to your DB
router.post('/city', function(req, res) {
    const newCity = new City(req.body)
    newCity.save().then(newCity=> {res.send(newCity)}).catch(err=> res.send(err))
})
...
router.delete('/city/:cityName', async (req, res)=> {
    const { deleteCity } = req.params /// is the parameter a name(not id) because city name is unique?
    const city = await City.findOneAndDelete(deleteCity)
    res.send(city)
})
